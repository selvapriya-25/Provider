...
var spec_join = provspec.as("p").join(spec.as("s"), upper(trim(col("p.specialtycode"))) === upper(trim(col("s.specialtycode"))) && upper(trim(col("p.part_state"))) === upper(trim(col("s.part_state"))), "inner").select(col("p.provid"), col("p.specialtycode"), col("p.part_state"), col("p.spectype"), col("p.lastupdate"), col("s.description"), col("p.effdate"), col("p.termdate"), col("p.createdate"), col("p.specialtystatus")) // to test updated left to inner
